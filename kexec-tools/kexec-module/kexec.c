...
long int (*my_memblock_reserve)(phys_addr_t, phys_addr_t);
...
printk("Kexec : kimage_load_normal_segment - Start.....\n");
	result = kimage_set_destination(image, maddr);
	if (result < 0)
{printk("Kexec : kimage_load_normal_segment - Error set destination.....\n");
		goto out;
}
	...
	while (mbytes) {
		...
		if (!page) {
			result  = -ENOMEM;
printk("Kexec : kimage_load_normal_segment - Error alloc page.....\n");
			goto out;
		}
		result = kimage_add_page(image, page_to_pfn(page)
								<< PAGE_SHIFT);
		if (result < 0)
{printk("Kexec : kimage_load_normal_segment - Error add page.....\n");
			goto out;
}
...
printk("Kexec : kimage_load_normal_segment - Error copy from user.....\n");
			goto out;
		}
		ubytes -= uchunk;
		maddr  += mchunk;
...
out:
printk("Kexec : kimage_load_normal_segment - End.....\n");
	return result;
}
...
//asmlinkage int kexec_load(unsigned long entry, unsigned long nr_segments, struct kexec_segment __user *segments, unsigned long flags)
asmlinkage int my_kexec_load(unsigned long entry, unsigned long nr_segments, struct kexec_segment __user *segments, unsigned long flags)
{
...
{printk("Kexec: - (flags & KEXEC_FLAGS) : '0x%lx'\n",(flags & KEXEC_FLAGS));
 printk("Kexec: - (flags & ~KEXEC_ARCH_MASK) : '0x%lx'\n",(flags & ~KEXEC_ARCH_MASK));
		return -EINVAL;
}
*/
...
#include <asm/memblock.h>
	...
	#define KEXEC_HARDBOOT_START	0x78000000
	...
	static void kexec_hardboot_reserve(void)
	{
		...
		if (memblock_reserve(KEXEC_HARDBOOT_START, KEXEC_HARDBOOT_SIZE))
			{
			printk(KERN_ERR "Kexec : Failed to reserve memory for KEXEC_HARDBOOT with size 0x%X at 0x%.8X\n", KEXEC_HARDBOOT_SIZE, KEXEC_HARDBOOT_START);
			return;
			}
...
/////////////
/////////////
// Reservation for "relocate_new_kernel"
	extern const unsigned char relocate_new_kernel[];
	extern const unsigned char relocate_new_kernel_end[];
	...
	static struct resource kexec_RelocateK_resources[] = 
	{
		[0] = 
			{
			.start  = relocate_new_kernel,
			.end    = relocate_new_kernel_end,  // -1 ??
			.flags  = IORESOURCE_MEM,
			},
	};
	...
	static void kexec_RelocateK_reserve(void)
	{
		//memblock_init();
//arm_memblock_init();
		//memblock_free(relocate_new_kernel, relocate_new_kernel_size);
		//memblock_remove(relocate_new_kernel, relocate_new_kernel_size);
		my_memblock_reserve = (long int *)kallsyms_lookup_name("memblock_reserve");
		if (my_memblock_reserve(relocate_new_kernel, relocate_new_kernel_size))
			{
			printk(KERN_ERR "Kexec : Failed to reserve memory for KEXEC_RELOCATE_KERNEL with size 0x%X at 0x%.8X\n", relocate_new_kernel_size, relocate_new_kernel);
			return;
			}
...
void hijack_start ( void *target, void *new )
{
    ...
    *(unsigned long *)&n_code[8] = (unsigned long)new;
    pr_info("Kexec : Hooking function 0x%p with 0x%p\n", target, new);
...
static int __init kexec_module(void)
{
	...
	//  grep xxxxxxx /proc/kallsyms
	//
	//  Use "mem_text_write_kernel_word" to avoid protection faults
	// (writes to read-only kernel memory) when CONFIG_STRICT_MEMORY_RWX is enabled.
	//

	/* alocate mem space for kexec hardboot atags */
	//kexec_hardboot_reserve();
	//platform_add_devices(kexec_devs, ARRAY_SIZE(kexec_devs));
	
//Hijack Start
	hijack_start((void *)kallsyms_lookup_name("sony_ric_enabled"), &sony_ric_enabled);
	
	
/* alocate mem space for kexec relocate_kernel */
//	kexec_RelocateK_reserve();
//	platform_add_devices(kexec_RKdevs, ARRAY_SIZE(kexec_RKdevs));

	
	my_syscall_table = (void **)kallsyms_lookup_name("sys_call_table");
	/*debug*/printk("Kexec_load: Replacement... : \n");
	/*debug*/printk("kexec_load : my_syscall_table : %p\n", my_syscall_table);

	/* Set kexec_load() syscall. */
	/*debug*/printk("kexec_load : kexec_load before replacement : %p\n", my_syscall_table[__NR_kexec_load]);
	mem_text_write_kernel_word((long unsigned int *)&my_syscall_table[__NR_kexec_load], (long unsigned int)my_kexec_load);
	/*debug*/printk("kexec_load : kexec_load after replacement : %p\n", my_syscall_table[__NR_kexec_load]);

	/* Swap reboot() syscall and store original */
	/*debug*/printk("kexec_load : reboot before replacement : %p\n", my_syscall_table[__NR_reboot]);
	original_reboot=my_syscall_table[__NR_reboot];
	mem_text_write_kernel_word((long unsigned int *)&my_syscall_table[__NR_reboot], (long unsigned int)reboot);
	/*debug*/printk("kexec_load : reboot after replacement : %p\n", my_syscall_table[__NR_reboot]);
	/*debug*/printk("Kexec_load: End replacement... : \n");
	///////////////////////////////////////////////
 ...
 */
static void kexec_my_exit(void)
{
        printk(KERN_ALERT "Kexec : Exit kernel module.... Goodbye !\n");
}

module_init(kexec_module);
module_exit(kexec_my_exit);
