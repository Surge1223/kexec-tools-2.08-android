...
#define MEMSCANI fprintf(stderr,"\n***Start scan\n");for (ISC=0;ISC<sizescan;ISC++){fprintf(stderr,"%.Lx ",scanmem++);}fprintf(stderr,"\n***End scan\n\n");
...
fprintf(stderr, "INFO : Segment[%d] - BUF - Start : %p  - length : %p\n",i,(char *)info->segment[i].buf,(void *)info->segment[i].bufsz);
fprintf(stderr, "INFO : Segment[%d] - MEM - Start : %p  - length : %p  - end : %p\n",i,(char *)info->segment[i].mem,(void *)info->segment[i].memsz,end);
	}
	return 0;
}
...
// This function is the real "add_segment()"
// See for source to dest addresses
	...
	unsigned long base, size_t memsz, int phys)
{
	...
	if (memsz == 0) {
		fprintf(stderr, "Kexec : add_segment_phys_virt - empty segment...\n");
		return;
	}
	 ...
	 */
	if (base & (pagesize -1)) {
		fprintf(stderr, "kexec :  base = '0x%lx',base\n");
		fprintf(stderr, "kexec :  (pagesize -1) = '0x%lx'\n",(pagesize -1));
	...
	if (info->nr_segments > KEXEC_MAX_SEGMENTS) {
		fprintf(stderr, "Warning: kernel segment limit reached. This will likely fail\n");
	}
}
...
fprintf(stderr, "k_unload - kexec_load - function not imlemented !!\n");
...
fprintf(stderr, "k__jump_back_helper_image - kexec_load - function not imlemented !!\n");
